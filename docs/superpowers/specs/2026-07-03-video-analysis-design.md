...
在 class-lab 中新增「视频分析」模块：特教老师上传或粘贴一段 **5–30 分钟** 的课堂视频，
...
- 校验：文件大小上限（默认 ≤ 500MB）、时长 5–30 分钟（超出提示但仍尝试，或直接拒绝——取保守：**超出范围给明确提示并拒绝**）、类型须为视频。
...
- **文件校验**：超大（>500MB）、时长超出 5–30 分钟、非视频类型 → 明确提示并拒绝。
