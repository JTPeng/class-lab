...
import { z } from 'zod';
import { loginOrCreate } from '../db/users.js';
...
const LoginSchema = z.object({ username: z.string().trim().min(1).max(40) });
...
export async function registerUserRoutes(app: FastifyInstance, deps: UserRoutesDeps) {
  ...
  // 轻量登录：没有该用户名则自动创建，返回用户。
  app.post('/auth/login', async (request, reply) => {
    const parsed = LoginSchema.safeParse(request.body);
    if (!parsed.success) {
      return reply.status(400).send({ error: '用户名不能为空', issues: parsed.error.issues });
    }
    const user = loginOrCreate(db, parsed.data.username);
